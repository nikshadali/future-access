       ...
       <Accordion type="single" collapsible className=" flex flex-col gap-5 "
        ...
        >
          ...
          {faqs.map((faq, index) => (
            ...
            //   </AccordionContent>
            // </AccordionItem>
            <AccordionItem key={index} value={`item-${index}`} className="group bg-primary rounded-4xl  border-0"
              style={{}}
            >
              <AccordionTrigger
  className={`font-bold w-full rounded-none text-white py-6 text-[20px] hover:no-underline group-hover:text-accent [&>svg]:hidden px-5 ${openItem === `item-${index}` ? "border-b border-accent-foreground/50 text-accent" : ""}`}
>
  <div className="flex items-center gap-2">
    <span className={`${openItem === `item-${index}` ? "text-accent" : "text-accent-foreground/50"} group-hover:text-accent`}>{index + 1}</span>
    {faq.question}
  </div>
  ...
  >
    <Plus size={22} className="text-white transition-transform duration-300" />
  </span>
</AccordionTrigger>

              <AccordionContent className={`text-accent-foreground/90 text-[18px] px-5  py-6 `}>
                {faq.answer}
              </AccordionContent>
            </AccordionItem>
          ))}
